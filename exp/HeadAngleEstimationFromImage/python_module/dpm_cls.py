...
def head_pose_estimation(headimage_, image, location_, DEPLOY='deploy_new', PRETRAINED='model_new', MEAN_FILE='mean_new.npy'):
	...
	net = caffe.Classifier(DEPLOY, PRETRAINED,
...
for module
"""
def classify_byimage(detect_mfile, image_file, output, model, deploy, mean):
	...
	for c, candidate in enumerate(detections_array):
		for i in xrange(8):
	...
	head_pose_estimation(head_image, image, location, deploy, model, mean)
...
def main():
	...
	for imagename in filenames:
		...
		# cv2.imshow(window_name, image)
		cv2.imwrite("image_new/"+os.path.basename(imagename), image)
		frame_num += 1
