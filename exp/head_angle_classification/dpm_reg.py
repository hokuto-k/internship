...
def head_pose_estimation(headimage_, image, location_):
	...
	head_image = skimage.img_as_float(headimage_).astype(np.float32)
	...
	# predict_max = np.argmin(prediction[:,0])
	...
	draw_arrow(image, location, 80, predict_angle * 2 * np.pi / 360.0)
...
def main():
	...
	for imagename in filenames:
		...
		for c, candidate in enumerate(detections_arrays[frame_num]):
			...
			head_image = image[candidate_array[c][5]:candidate_array[c][5]+candidate_array[c][7], candidate_array[c][4]:candidate_array[c][4]+candidate_array[c][6]]
			try:
				head_pose_estimation(head_image, image, [candidate_array[c][4], candidate_array[c][5]])
			except:
		...
		# cv2.imshow(window_name, image)
		cv2.imwrite(os.path.basename(imagename), image)
		frame_num += 1
