...
def configure_data_layer(task):
    ...
    This task takes following inputs:
        0. Config prototxt file (source, batchsize, cropsize and mirror can be
           omitted)
        1. Source dataset for training
        2. Source dataset for test
        3. Mean file (optional)
    ...
    if 'train_batchsize' in task.parameter:
        net.layers[0].data_param.batch_size = task.parameter['batchsize']

    if 'test_batchsize' in task.parameter:
        net.layers[1].data_param.batch_size = task.parameter['batchsize']
...
def create_solver(task):
    ...
    print type(solver.test_net), type(solver.train_net)
    solver.train_net = task.inputs[0].abspath()
    solver.test_net[0] = task.inputs[1].abspath()
